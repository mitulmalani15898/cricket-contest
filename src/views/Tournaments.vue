    ...
    <v-container>
        <v-dialog v-model="dialog" persistent max-width="500px">
            <template slot="activator">
                <v-btn color="success" @click="modalType='add'">Add Tournament</v-btn>
            </template>
            <v-card>
                <v-form ref="form" lazyValidation v-model="valid" @submit.prevent="closeAndSaveDialog">
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="green darken-1" v-if="modalType === 'update'" @click="updateTournament" flat>Update</v-btn>
        ...
        <v-dialog v-model="dialogDelete" max-width="290">
            <v-card class="text-xs-center dialog-delete">
                ...
                <v-card-actions>
                    <v-btn color="green darken-1" flat="flat" @click="deleteTournament">
                        Yes
                    </v-btn>
        ...
        <v-dialog v-model="dialogTeams" scrollable max-width="450">
            <v-card>
                <v-toolbar dark color="primary" class="dialog-toolbar">
                    <v-toolbar-title>{{ previousValues.tournamentName }}</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn icon dark @click="dialogTeams = false; selected = []">
                            <v-icon>mdi-close</v-icon>
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <div v-if="teams && !teams.length">No teams found in {{ previousValues.tournamentName }}</div>
                    <v-card-actions style="justify-content: flex-end">
                        <v-btn color="blue" @click="addTeamDialog">Add Teams</v-btn>
                    </v-card-actions>
                    <div v-if="teams && teams.length">
                        ...
                        <v-btn color="red" style="justify-content: space-between" dark @click="deleteTeams"
        ...
        <v-dialog v-model="dialogAddTeams" scrollable max-width="450">
            <v-card height="450">
                <v-toolbar dark color="primary" class="dialog-toolbar">
                    <v-toolbar-title>Tournament</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn icon dark @click="dialogAddTeams = false">
                            <v-icon>mdi-close</v-icon>
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                ...
                <v-card-actions style="justify-content: flex-end">
                    <v-btn color="blue" @click="addTeam">Submit</v-btn>
                    <v-btn color="blue" @click="dialogAddTeams = false; dialogTeams = true; value = []">Cancel</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        >
            <template slot="items" slot-scope="props">
                <tr>
                    <td class="text-xs-center"><img :src="thumbnailPath + props.item.tournamentBanner" alt=""></td>
                    <td class="text-xs-center">{{ props.item.tournamentName }}</td>
                    <td class="text-xs-center">{{ props.item.tournamentDescription}}</td>
                    <td class="text-xs-center">
                        <v-btn color="green darken-1" @click.stop="showTeams(props.item)">Teams</v-btn>
                    </td>
                    <td class="text-xs-center">
        ...
        data () {
            return {
                headers: [
                    ...
                    {text: 'Description', value: 'tournamentDescription', align: 'center'},
                ...
                descriptionRules: [
                    v => !!v || 'Tournament description is required',
                ],
                valid: true,
                pagination: {},
                isLoading: false,
        ...
        methods: {
            update(item) {
                ...
                if (item.tournamentBanner !== 'defaultTournament.png') {
                    this.imagePath = thumbnailPath + item.tournamentBanner
                }
                this.dialog = true
            },
            pickFile() {
            ...
            updateTournament() {
                ...
                formData.append('tournamentName', this.name)
                formData.append('tournamentDescription', this.description)
                this.imageFile ? formData.append('tournamentBanner', this.imageFile) : formData.append('tournamentBanner', 'defaultTournament.png');
...
<style scoped>
    ...
    .tournamentTable >>>th.column.text-xs-center, th.column.text-xs-center.sortable:hover {
        font-size: 15px;
